...
@RequestMapping(value="register.htm", method=RequestMethod.POST)
    public ModelAndView AddS(dbModel p){
    String sql = "INSERT INTO products (name, description, image, price) VALUES (?, ?, ?, ?)";
    this.jdbctemp.update(sql, p.getName(), p.getDescription(), p.getImage(), p.getPrice());
  ...
  Datas=this.jdbctemp.queryForList(sql);
   ...
   this.jdbctemp.update(sql, product.getName(),product.getDescription(),product.getPrice(),product.getImage(),product.getId());

   
   return new ModelAndView("adminIndex.htm");        
   ...
   @RequestMapping(value="delet.htm")
   public ModelAndView delet( @RequestParam("id")int id){
...
public ModelAndView processPayment(HttpServletRequest request) {
   ...
   // List<Map<String, Object>> cartItems = null;
    ...
    jdbcTemplate.update(insertSql, userId, username, email, paymentMethod, mobileNumber, String.valueOf(totalPrice), productNames);
